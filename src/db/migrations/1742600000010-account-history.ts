...
export const up = async (db: Kysely<any>): Promise<void> => {
  await sql`CREATE TYPE account_history_field AS ENUM ('email', 'username')`.execute(db);

  await db.schema
    .createTable('account_history').ifNotExists()
  ...
  await db.schema
    ...
    .addForeignKeyConstraint('account_history_account_id_fk', ['account_id'], 'accounts', ['id'], fk => fk.onUpdate('cascade').onDelete('cascade'))
    .execute();
